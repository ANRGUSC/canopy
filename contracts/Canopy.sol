...
contract Canopy is Chainlinked {
    ...
    function requestEthereumPrice(string _currency) public {
        bytes32 jobId = bytes32("2216dd2bf5464687a05ded0b844e200c");
        ...
        run.add("url", "http://neptune.usc.edu:5000/convert?curr=USD");
        string[] memory path = new string[](1);
        path[0] = "value";
        run.addStringArray("path", path);
        run.addInt("times", 1);
    ...
    function makeDeposit() public payable {
        buyerEscrow = msg.value;
        buyer = msg.sender;
    }

    // For seller to view amount that buyer has in escrow
    function viewDeposit() public constant
        onlyBy(owner)
        returns (uint)
    {
        return buyerEscrow;
    ...
    // For seller to get payment from buyer
    function chargeAmount(uint _cost, string _ccy) public
        onlyBy(owner)
    {
        _ccy; //TODO: call ChainLink for currency conversion
        uint amount = _cost;
    ...
    // For buyer to withdraw their funds
    function withdraw() public
        onlyBy(buyer)
    {
        buyer.transfer(buyerEscrow);
        buyerEscrow = 0;
        buyer = 0;
    }
}
